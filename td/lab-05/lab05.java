...
// kod zadania 5 mojego autorstwa, chat zostal poproszony o zmiane/poprawe kodu
...
//        PlotTwoSignals(msig, f3sig,"sygnal i modyfikacja czestotliwosci k =" + freqVals[2] ,"czas","amp");
        PlotNineSignalsInOneWindow(z1sig,z2sig,z3sig,p1sig,p2sig,p3sig,f1sig,f2sig,f3sig,"sygnaly po modulacji","czas","amp");

        // LAB05
        XYSeries[] series = new XYSeries[9];
        double[][] real = new double[9][N];
        double[][] imag = new double[9][N];

        double[] B = {3.0,6.0,10.0};


        String[] nazwy = { "M1", "M2", "M3", "M4", "M5", "M6", "M7", "M8", "M9" };
        ...
        for (int j = 0; j < N / 2; j++) {
            ...
            for (int i = 0; i < 9; i++) {
                ...
                double magDB = 10 * Math.log10(mag + 0.0000000001);
                magnitude[i][j] = magDB;
        ...
        for(int k = 0;k < 9;k++){
            System.out.println("Maksymalna wartosci widma amplitudowego " + (k+1) + " : " + maxVal[k]);
        }
        for (int i = 0; i < 9; i++) {
            double h = maxVal[i] - B[2];
            int leftIndex = -1;
            int rightIndex = -1;
            ...
            if (leftIndex != -1 && rightIndex != -1) {
                double fLeft = leftIndex * (sampleFreq / N);
                double fRight = rightIndex * (sampleFreq / N);
                double width = fRight - fLeft;
                System.out.printf("Szerokość pasma sygnału %d (-"+B[2] + "dB): %.2f Hz\n", i + 1, width);
            }
        }


        PlotNineSignalsInOneWindow(
                series[0], series[1], series[2], series[3], series[4],
                series[5], series[6], series[7], series[8],
                "widma amplitudowe", "freq", "amp [dB]"
        );
    ...
    public static void PlotTwoSignals(XYSeries series1, XYSeries series2, String title, String xLabel, String yLabel) {
        ...
        JFreeChart chart = ChartFactory.createXYLineChart(
                title, xLabel, yLabel,
                dataset, PlotOrientation.VERTICAL,
                true, true, false // legend is set to true to distinguish both signals
        );
    ...
    public static void PlotNineSignalsInOneWindow(
        ...
        // Create main panel with 3x3 grid layout
        ...
        // Display all charts in one JFrame
